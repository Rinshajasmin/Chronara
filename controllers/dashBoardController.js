...
const getDashBoard = async (req, res) => {
  try {
    ...
    const todaysSales = todaysOrders
      .reduce((acc, order) => acc + order.finalAmount, 0)
      .toFixed(2); // Total revenue today
    const todaysDiscount = todaysOrders.reduce(
      (acc, order) => acc + order.discounts,
      0
    ); // Total discounts today
    const todaysRevenue = (
      parseFloat(todaysSales) - parseFloat(todaysDiscount)
    ).toFixed(2);
    ...
    const totalBrandSales = brandSales.reduce(
      (acc, curr) => acc + curr.totalQuantitySold,
      0
    );
    brandSales.forEach((brand) => {
      brand.percentage = Math.round(
        (brand.totalQuantitySold / totalBrandSales) * 100
    ...
    categorySales.forEach((category) => {
      ...
      productSales.forEach((product) => {
        product.percentage = Math.round(
          (product.totalQuantitySold / totalProductSales) * 100
    ...
    res.render("admin/dashBoard", {
      ...
      defaultRevenueData: monthlyRevenueData.map((val) =>
...
const getFilteredRevenue = async (req, res) => {
  try {
    ...
    const selectedYear = parseInt(year); // Convert year to number
    ...
    if (filterType === "yearly") {
      ...
      res.json({
        ...
        monthlyRevenueData: monthlyRevenueData.map((val) =>
          parseFloat(val.toFixed(2))
        ),
      });
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dailyTotal = dailyOrders.reduce(
          (sum, order) => sum + (order.finalAmount - order.discounts),
          0
        );
